...
class PGAgent(nn.Module):
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            for reward_list in rewards:
                discounted_returns = self._discounted_return(reward_list)
            ...
            q_values = []
            for reward_list in rewards:
                discounted_returns = self._discounted_reward_to_go(reward_list)
                q_values.append(discounted_returns)
        ...
        if self.critic is None:
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    # the GAE(s_t,a_t) = delta_t + gamma * lambda * GAE(s_{t+1},a_{t+1}) 
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for i, reward in enumerate(rewards):
            sum += self.gamma ** i * reward
        discounted_return = np.array([sum]*len(rewards))
